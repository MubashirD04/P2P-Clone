    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as tracker_socket:
            ...
            try:
                ...
                logging.warning(f"No response from tracker when registering {filename}")
    ...
    while True:
        ...
        for filename in registered_files.copy():
            if not os.path.exists(filename):
                ...
                logging.warning(f"Removed non-existent file: {filename}")
                continue
...
def register_available_files(port):
    # Register all seeded files with the tracker
    files = []
