        ...
        while True:
            try:
                ...
                if message[0] == "REGISTER":
                    # Expected format: REGISTER <filename> <port>
                    if len(message) != 3:
                    ...
                    if filename in seeder_list:
                        ...
                        if active_seeders:
                            ...
                            response = "NO_SEEDERS"
                            logging.warning(f"⚠️ No active seeders available for {filename}.")
                    else:
                        response = "NO_SEEDERS"
                        logging.warning(f"⚠️ No seeders found for {filename}.")

                    tracker_socket.sendto(response.encode(), addr)

                elif message[0] == "HEARTBEAT":
                    # Expected format: HEARTBEAT <filename>
                    if len(message) != 2:
                    ...
                    if filename in seeder_list:
                        ...
                        for i, (ip, port, _) in enumerate(seeder_list[filename]):
                            if ip == addr[0]:
                                seeder_list[filename][i] = (ip, port, time.time())  # Update timestamp
                                updated = True
                                logging.info(f"💓 Heartbeat received from {addr[0]} for {filename}")
                        
                        if not updated:
                            logging.warning(f"⚠️ Heartbeat received from non-registered seeder {addr[0]} for {filename}")
                    else:
                        logging.warning(f"⚠️ Heartbeat received for non-registered file {filename} from {addr[0]}")
...
def remove_inactive_seeders():
    while True:
        ...
        for filename, seeders in seeder_list.items():
            ...
            if len(active_seeders) != len(seeders):  # If some are removed
                logging.warning(f"❌ Removed inactive seeders for {filename}. Active seeders: {len(active_seeders)}")

            if active_seeders:
                seeder_list[filename] = active_seeders  # Keep only active seeders
            else:
                del seeder_list[filename]  # No seeders left
